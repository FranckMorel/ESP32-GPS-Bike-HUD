...
void setup() {
  ...
  Serial.println("PA1010D I2C Test (ESP32)");

  if (!GPS.begin(0x10)) {
    Serial.println("GPS nicht gefunden (Addr 0x10) -> Verkabelung checken!");
    while (1) delay(10);
  }
  ...
  Serial.println("GPS gestartet, warte auf Fix...");
}
...
void loop() {
  ...
  if (GPS.newNMEAreceived()) {
    ...
    // Optional zum Debuggen: rohe NMEA anzeigen
  ...
  if (millis() - t >= 1000) {
    t = millis();
    Serial.print("Sats: "); Serial.print(GPS.satellites);
    Serial.print("  Fix: "); Serial.print((int)GPS.fix);
    if (GPS.fix) {
      Serial.print("  Lat: ");   Serial.print(GPS.latitudeDegrees, 6);
      Serial.print("  Lon: ");   Serial.print(GPS.longitudeDegrees, 6);
      Serial.print("  v: ");     Serial.print(GPS.speed * 1.852, 2); // km/h
      Serial.print(" km/h");
    }
    Serial.println();
  }
